...
const HeadIcons = ({ type }) => {
  return (
    <span>
      {iconsMap[type] || <div className="text-gray-500">Icon Not Found</div>}
    </span>
  );
};
...
export default function OpenSidebar({ showSidebar }) {
  ...
  const generateLink = (item, refName) => (
    <a
      id={refName}
      key={item.id}
      onClick={(e) => {
    ...
    >
      <span>{item.icon || <HeadIcons type={item.type}  />}</span>
  ...
  return (
    <div className="mt-5 p-2 mb-4 -m-3 w-[250px] max-h-screen rounded-r-md">
      ...
      <ul className="mt-2 flex flex-col">
        {menus?.map((menu) => generateLink(menu, `menu-${menu.id}`))}
      </ul>
      <ul className="mt-4 flex flex-col">
        ...
        {listMenu?.map((lists) => generateLink(lists, `list-${lists.id}`))}
      </ul>
      <ul className="mt-4 flex flex-col">
        {showSidebar && (
          <h2 className="font-Quicksand font-bold text-gray-600 dark:text-gray-300">
        ...
        {filterLists?.map((filter) => generateLink(filter, `filter-${filter.id}`))}
      </ul>
      {basket?.map((filter) => generateLink(filter, `basket-${filter.id}`))}
    </div>
  );
}
